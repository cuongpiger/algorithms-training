...
int main() {
	...
	double min_girl_cup = cups[0];
	double min_boy_cup = cups[n];
	double ratio = min_boy_cup / min_girl_cup;

	if (ratio >= 2) {
		x = min(x, min_girl_cup);
	}
	else {
		x = min(x, min_boy_cup / 2);
	}

	cout << fixed << setprecision(6) << (x * n * 3) << endl;
